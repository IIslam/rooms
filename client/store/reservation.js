...
export const mutations = {
    ...
    PUSH_UPDATED_RESERVATION(state, reservation) {
        state.reservations = state.reservations.filter(r => r._id != reservation._id)
        state.reservations.push(reservation)
    },
    UNSET_FORM(state) {
        state.form = {
...
export const actions = {
    index({commit}) {
        return this.$api.reservation().index().then((reservations) => {
            commit('SET_RESERVATIONS', reservations)
            return reservations
        }).catch(() => {})
    },
    show({commit}, reservationId) {
        return this.$api.reservation().show(reservationId).then((reservation) => {
    ...
    destroy({ dispatch }, reservationId) {
        return this.$api.reservation().destroy(reservationId).then(() => {
            dispatch('auth/unsetReservation', reservationId, { root: true })
        }).catch(() => {})
    },
    update({ commit, getters }, payload) {
        return this.$api.reservation().update(payload.reservationId, payload.roomId, getters.form).then(reservation => {
